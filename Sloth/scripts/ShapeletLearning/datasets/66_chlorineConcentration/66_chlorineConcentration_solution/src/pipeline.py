...
def process_test_view(dspath, prpath):
	...
	y_pred_df = pd.DataFrame(index=testDataDF.index, data=y_pred, columns=targetCols)
...
if __name__ == '__main__':
	...
	if VIEW == 'TRAIN':
		...
		accuracy, f1Macro = compute_performance(dspath, prpath, y_pred)
		df = pd.DataFrame(columns=['metric', 'value'])
		df.loc[len(df)] = ['accuracy', accuracy]
		df.loc[len(df)] = ['f1Macro', f1Macro]
